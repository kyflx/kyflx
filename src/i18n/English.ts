...
import {Collection, GuildMember, Message, Role, TextChannel} from "discord.js";
import ms from "ms";
			...
			cmds: {
				...
				conf: {
					auto: {
						...
						already: "These roles are already being used, if you want to remove them do `ar rm <role>`",
						set: (roles: Role[]) =>
					...
					lum: {
						desc: {
							...
							examples: ["v!lum redirect #levelup", "v!lum enable", "v!lum disable", "v!lum reset"]
						},
						prompt: "Please provide a channel I can redirect level up messages to.",
						curr: (message: Message) => !message._guild.lvlUpMsg
							? `Level up messages are currently disabled, do \`lum enable\` to enable them`
							: `Level up messages are enabled${message._guild.lvlUpChannel
						...
						red: (channel: TextChannel) => `Okay, I am now redirecting level up messages to ${channel} (\`${channel.id}\`)`,
						enable: "Okay, I enabled level up messages! Do `lum disable` to disable them again.",
						disable: "Okay, I disabled level up messages! Do `lum enable` to enable them again."
					},
					mtr: {
						desc: {
							content: [
