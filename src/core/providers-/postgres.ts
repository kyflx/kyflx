// import { SQLProvider, QueryBuilder, Type } from "klasa";
...
// 	public db: Pool;
// 	public dbconnection: PoolClient;

// 	constructor(store: any, file: any, directory: any, options: any) {
// 		super(store, file, directory, options);
// 		this.qb = new QueryBuilder({
// 			boolean: 'BOOL',
// 			float: 'DOUBLE PRECISION',
// 			uuid: 'UUID',
// 			json: { type: 'JSON', resolver: (input: any) => `'${JSON.stringify(input)}'::json` as any },
// 			any: { type: 'JSON', resolver: (input: any) => `'${JSON.stringify(input)}'::json` as any },
// 			// @ts-ignore
// 			integer: ({ max }: any) => max !== null && max >= 2 ** 32 ? 'BIGINT' : 'INTEGER',
// 			// @ts-ignore
// 			array: (type: any) => `${type}[]`,
// 		}, { 
// 			arrayResolver: (values: any[], piece: any, resolver: any) => values.length ? `array[${values.map(value => resolver(value, piece)).join(', ')}]` : "'{}'",
// 			formatDatatype: (name: any, datatype: any, def: any = null) => `"${name}" ${datatype}${def !== null ? ` NOT NULL DEFAULT ${def}` : ''}`
// 		});
// 		this.db = null;
// 	}
...
// 	public createTable(table: string, rows: [string, any][]) {
...
// 	public getAll(table: string, entries: string[] = []) {
// 		if (entries.length) {
...
// 	public get(table: string, key, value) {
...
//  * @private
//  */
// function sanitizeKeyName(value: any) {
