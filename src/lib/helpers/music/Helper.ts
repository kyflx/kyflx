...
import { Kyflx, Util } from "../../";
import { FilterMap } from "../../../types";
import { Playlist, Queue, REST, Result, Song, SongSource } from "./lib";
...
class KyflxMusic extends Plugin {
  public preRegister(): void {
    ...
    class KyflxPlayer extends Structures.get("player") {
      ...
      public filter<K extends keyof FilterMap>(
        type: K,
        data: FilterMap[K],
        merge: boolean = false
      ) {
        let payload: Partial<FilterMap> = { [type]: data };
        Object.assign(this.filters, payload);
        return this.send(
          "filters",
          merge ? Object.assign(payload, this.filters) : payload
        );
      }
    }
  }
...
export class MusicHelper extends Manager {
  ...
  public constructor(public kyflx: Kyflx) {
    ...
    this.#sources = new Collection([
      [ "spotify", { source: S.Spotify, song: S.SpotifySong } ],
  ...
  public async load<T = Song | Playlist>(
    ...
    if (_ && _.length > 0) {
      ...
      if (_.searchResult || _.length === 1) {
        const [ song ]: any = _,
          source = (song.info.source = Util.getSource(song.info));
        ...
        const info = Util.getPlaylist(identifier);
