...
export default class extends Command {
  ...
  public async exec(message: Message) {
    const { level, xp } = message.profile,
      users = (
        await ProfileEntity.find({
          where: { guildId: message.guild.id }
        })
      ).sort((a, b) => b.xp - a.xp),
      rank = users.findIndex(user => user.userId === message.author.id) + 1;

    const canvas = createCanvas(500, 200);
    const ctx = canvas.getContext("2d");
    const xpNeed = level * 75 * 2;
    const image = await loadImage(join(process.cwd(), "assets/rank-card.png"));
    ...
    return message.util.send({ embed, files: [attachment] });
  }
}
